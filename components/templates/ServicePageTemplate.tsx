...
// ─────────────────────────────────────────────────────────────────────────────
// Canonical silo schema (REQUIRED core fields) + optional spark-rich fields.
//
// The core fields — slug, badge, title, headline, description, bullets, benefits,
// stats?, faqs?, relatedLinks? — are exactly what the QA silo builder emits when
// it appends `{...BASE, ...}` entries to lib/data/services.ts. Keep them required
// so silo-generated pages render correctly.
//
// The optional fields (heroImage, featuredVideo, intro, signs, approach,
// premiumOptions, timeline, videoLibrary) let the hand-authored pages keep their
// full bespoke layout. A silo-appended entry that only sets the core fields still
// renders a complete, styled page — it just omits the rich sections it lacks.
// ─────────────────────────────────────────────────────────────────────────────
...
export type ServicePageContent = {
  ...
  // Catch-all for page-specific prose blocks that don't fit a named section
  // (e.g. "Specialized Diagnostic Technologies", "Stages We Monitor"). Preserves
  // bespoke content during the data-driven migration without a custom template.
...
export function ServicePageTemplate({ c }: { c: ServicePageContent }) {
  ...
  return (
    <main className="min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col lg:flex-row gap-8 lg:gap-10 items-center">
            <div className="w-full lg:w-3/5">
              ...
              <h1 className="font-['Cormorant'] text-5xl md:text-6xl font-light mb-6 leading-tight">{c.headline}</h1>
      ...
      {c.featuredVideo && (
        <section className="bg-white py-16">
          <div className="max-w-4xl mx-auto px-6">
            {c.featuredVideo.heading && (
              <h2 className="font-['Cormorant'] text-3xl md:text-4xl mb-3 text-[var(--color-ink)] text-center">{c.featuredVideo.heading}</h2>
            )}
            {c.featuredVideo.subhead && (
              <p className="text-center text-[var(--color-muted)] mb-8 max-w-2xl mx-auto">{c.featuredVideo.subhead}</p>
            )}
            <YouTubeEmbed videoId={c.featuredVideo.videoId} title={c.featuredVideo.title} />
          </div>
        </section>
      )}

      {/* What is it + signs */}
      {(c.intro?.length || c.signsList?.length) && (
        <section className="bg-[var(--color-cream)] py-20">
          <div className="max-w-4xl mx-auto px-6">
            {c.introHeading && <h2 className="font-['Cormorant'] text-4xl md:text-5xl mb-8 text-[var(--color-ink)]">{c.introHeading}</h2>}
            {c.intro?.length ? (
            ...
            {c.signsList?.length ? (
              <div className="mt-12 bg-white rounded-xl p-8 shadow-sm">
                ...
                <ul className="space-y-4">
                  {c.signsList.map((s, i) => (
                    <li key={i} className="flex items-start gap-3">{CHECK}<span className="text-[var(--color-ink)]">{s}</span></li>
                  ))}
                </ul>
              </div>
            ) : null}
      ...
      {/* What to expect — canonical bullets (always present) */}
      {c.bullets?.length ? (
        <section className="bg-white py-16">
          <div className="max-w-4xl mx-auto px-6">
            <h2 className="font-['Cormorant'] text-3xl md:text-4xl mb-8 text-[var(--color-ink)] text-center">What to Expect</h2>
            <ul className="space-y-4 max-w-2xl mx-auto">
              {c.bullets.map((b, i) => (
                <li key={i} className="flex items-start gap-3">{CHECK}<span className="text-[var(--color-ink)] text-lg">{b}</span></li>
              ))}
            </ul>
          </div>
        </section>
      ...
      {c.approach?.length ? (
        <section className="bg-white py-20">
          <div className="max-w-7xl mx-auto px-6">
            {c.approachHeading && <h2 className="font-['Cormorant'] text-4xl md:text-5xl mb-4 text-[var(--color-ink)] text-center">{c.approachHeading}</h2>}
            ...
            <div className="grid md:grid-cols-3 gap-8 mb-4">
              {c.approach.map((card, i) => (
                <div key={i} className="bg-[var(--color-cream)] rounded-xl p-8 hover:shadow-lg transition-shadow">
                  <div className="mb-6">
      ...
      {/* Premium / sub-options grid */}
      {c.premiumOptions?.length ? (
        <section className="bg-white pb-20">
          <div className="max-w-7xl mx-auto px-6">
            <div className="bg-[var(--color-light)] rounded-2xl p-10 max-w-4xl mx-auto">
              {c.premiumHeading && <h3 className="text-3xl font-['Cormorant'] mb-6 text-[var(--color-ink)]">{c.premiumHeading}</h3>}
      ...
      {/* Benefits — canonical (always present) */}
      {c.benefits?.length ? (
        <section className="bg-[var(--color-cream)] py-20">
          <div className="max-w-5xl mx-auto px-6">
            <h2 className="font-['Cormorant'] text-4xl md:text-5xl mb-12 text-[var(--color-ink)] text-center">Benefits</h2>
            <div className="grid md:grid-cols-2 gap-6">
              {c.benefits.map((b, i) => (
        ...
        <section className="bg-white py-20">
          <div className="max-w-3xl mx-auto px-6">
            <div className="bg-[var(--color-light)] rounded-2xl p-12">
              {c.timelineHeading && <h2 className="font-['Cormorant'] text-4xl mb-8 text-[var(--color-ink)] text-center">{c.timelineHeading}</h2>}
      ...
      {/* Extra bespoke prose sections */}
      {c.extraSections?.length ? (
        <section className="bg-[var(--color-cream)] py-20">
          <div className="max-w-4xl mx-auto px-6 space-y-14">
            {c.extraSections.map((s, i) => (
              <div key={i}>
                <h2 className="font-['Cormorant'] text-3xl md:text-4xl mb-6 text-[var(--color-ink)]">{s.heading}</h2>
      ...
      {/* Video library */}
      {c.videoLibrary?.length ? (
        <section className="bg-white py-20">
          <div className="max-w-6xl mx-auto px-6">
            {c.videoLibraryHeading && <h2 className="font-['Cormorant'] text-4xl md:text-5xl mb-4 text-[var(--color-ink)] text-center">{c.videoLibraryHeading}</h2>}
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {c.videoLibrary.map((v, i) => <YouTubeEmbed key={i} videoId={v.videoId} title={v.title} />)}
            </div>
          </div>
        </section>
      ) : null}
      ...
      {c.faqs?.length ? (
        <section className="bg-[var(--color-cream)] py-20">
          <div className="max-w-4xl mx-auto px-6">
            <h2 className="font-['Cormorant'] text-4xl md:text-5xl mb-12 text-[var(--color-ink)] text-center">Common Questions About {c.title}</h2>
      ...
      {c.relatedLinks?.length ? (
        <section className="bg-white py-16">
          <div className="max-w-7xl mx-auto px-6">
            <h3 className="text-3xl font-['Cormorant'] mb-10 text-[var(--color-ink)] text-center">Related {c.hubLabel}</h3>
      ...
      <section className="bg-gradient-to-br from-[var(--color-dark)] to-[var(--color-primary)] py-20 text-white text-center">
        <div className="max-w-4xl mx-auto px-6">
          <h2 className="font-['Cormorant'] text-4xl md:text-5xl mb-6">{c.ctaHeading || `Ready to Get Started?`}</h2>
